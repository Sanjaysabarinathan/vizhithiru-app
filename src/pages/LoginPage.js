      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
        {!isOtpSent ? (
          <>
            ...
            <button onClick={handleLogin} className="app-btn btn-blue" style={{ marginTop: 10 }}>Send OTP</button>
          </>
        ) : (
          <>
            <div style={{ textAlign: 'center', color: '#64748b' }}>OTP sent to {phone}</div>
            <input type="tel" placeholder="Enter 4-Digit OTP" value={otp} onChange={(e) => setOtp(e.target.value)} style={{ ...inputStyle, textAlign: 'center', letterSpacing: 5 }} />
            <button onClick={verifyOtp} className="app-btn btn-blue" style={{ marginTop: 10 }}>Verify & Login</button>
            <button onClick={() => setIsOtpSent(false)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer' }}>Change Number</button>
          </>
        )}
        {!isOtpSent && (
